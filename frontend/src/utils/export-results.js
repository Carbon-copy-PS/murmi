...
export function buildCSV(ctx) {
  ...
  if (!cg) return
    ...
    cg.essentialConditions.forEach((t) => row('Condition / red line', t))
    cg.unresolvedQuestions.forEach((t) => row('Unresolved issue', t))
    if (cg.changeSummary) row('What changed', cg.changeSummary)
...
export function buildPrintableHTML(ctx) {
  ...
  cgHistory.forEach((item, index) => {
    ...
    sections.push(`<section class="pdf-section pdf-cg">
      ...
      ${cg.essentialConditions.length ? `<div class="pdf-stance"><span class="pdf-stance-label">${esc(tr('cg.essentialConditions', 'Conditions / Red Lines'))}</span><ul>${cg.essentialConditions.map((t) => `<li>${esc(t)}</li>`).join('')}</ul></div>` : ''}
      ${cg.unresolvedQuestions.length ? `<div class="pdf-stance"><span class="pdf-stance-label">${esc(tr('cg.unresolvedQuestions', 'Unresolved Issues'))}</span><ul>${cg.unresolvedQuestions.map((t) => `<li>${esc(t)}</li>`).join('')}</ul></div>` : ''}
      ${cg.changeSummary ? `<div class="pdf-stance"><span class="pdf-stance-label">${esc(tr('cg.changeSummary', 'What changed'))}</span><p>${esc(cg.changeSummary)}</p></div>` : ''}
