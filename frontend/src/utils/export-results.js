...
function citedTexts(items) {
  return (items || [])
    .map((item) => {
      if (typeof item === 'string') return item
      if (item && typeof item === 'object' && item.text) {
        const eids = Array.isArray(item.evidenceIds) && item.evidenceIds.length
          ? ` [${item.evidenceIds.join(', ')}]`
          : ''
        return `${item.text}${eids}`
      }
      return ''
    })
    .filter(Boolean)
...
function commonGroundBlock(commonGround) {
  ...
  const endorsement = commonGround.endorsement
  const v = endorsement || commonGround.votes
  const total = v ? (v.agree || 0) + (v.disagree || 0) : 0
  const agreePct = total ? pct(v.agree, total) : 0
  const mode = resolveCgMode(commonGround)
  return {
    id: commonGround.id,
    generatedAt: commonGround.generatedAt,
    generatedByName: commonGround.generatedByName,
    mode,
    status: commonGround.status || 'working_draft',
    previousVersionId: commonGround.previousVersionId || null,
    changeSummary: (commonGround.changeSummary || '').trim(),
    ...
    tradeoffs: citedTexts(commonGround.tradeoffs),
    recommendations: citedTexts(commonGround.recommendations),
    essentialConditions: citedTexts(commonGround.essentialConditions),
    unresolvedQuestions: citedTexts(commonGround.unresolvedQuestions),
...
export function buildCSV(ctx) {
  ...
  if (!cg) return
    ...
    row('Analysis mode', tr(`cgMode.${cg.mode}.label`, cg.mode))
    if (cg.status) row('Status', cg.status)
    if (cg.previousVersionId) row('Previous version', cg.previousVersionId)
    if (cg.changeSummary) row('Change summary', cg.changeSummary)
    ...
    cg.recommendations.forEach((t) => row('Recommendation', t))
    cg.essentialConditions.forEach((t) => row('Essential condition', t))
    cg.tradeoffs.forEach((t) => row('Trade-off', t))
    cg.unresolvedQuestions.forEach((t) => row('Unresolved question', t))
  ...
  if (cgBlocks.length) {
    data.aiCommonGroundHistory = cgBlocks.map((cg) => {
      const entry = {
        ...
        mode: tr(`cgMode.${cg.mode}.label`, cg.mode),
        status: cg.status,
        statement: cg.statement,
      }
      if (cg.previousVersionId) entry.previousVersionId = cg.previousVersionId
      if (cg.changeSummary) entry.changeSummary = cg.changeSummary
      ...
      if (cg.recommendations.length) entry.recommendations = cg.recommendations
      if (cg.essentialConditions.length) entry.essentialConditions = cg.essentialConditions
      if (cg.tradeoffs.length) entry.tradeoffs = cg.tradeoffs
      if (cg.unresolvedQuestions.length) entry.unresolvedQuestions = cg.unresolvedQuestions
...
export function buildPrintableHTML(ctx) {
  ...
  cgHistory.forEach((item, index) => {
    ...
    const metaLine = [
      ...
      tr(`cgMode.${cg.mode}.label`, cg.mode),
      cg.status === 'endorsed' ? tr('cg.statusEndorsed', 'Final / endorsed') : tr('cg.statusDraft', 'Working draft'),
    ].filter(Boolean).join(' · ')
    sections.push(`<section class="pdf-section pdf-cg">
      ...
      ${cg.endorsementSummary ? `<p class="pdf-endorsement"><strong>${esc(cg.endorsementSummary)}</strong></p>` : ''}
      ${cg.changeSummary ? `<div class="pdf-stance"><span class="pdf-stance-label">${esc(tr('cg.changeSummary', 'Change summary'))}</span><p>${esc(cg.changeSummary)}</p></div>` : ''}
      ...
      ${cg.recommendations.length ? `<div class="pdf-stance"><span class="pdf-stance-label">${esc(tr('cg.recommendations', 'Recommendations'))}</span><ul>${cg.recommendations.map((t) => `<li>${esc(t)}</li>`).join('')}</ul></div>` : ''}
      ${cg.essentialConditions.length ? `<div class="pdf-stance"><span class="pdf-stance-label">${esc(tr('cg.essentialConditions', 'Essential conditions'))}</span><ul>${cg.essentialConditions.map((t) => `<li>${esc(t)}</li>`).join('')}</ul></div>` : ''}
      ...
      ${cg.unresolvedQuestions.length ? `<div class="pdf-stance"><span class="pdf-stance-label">${esc(tr('cg.unresolvedQuestions', 'Unresolved questions'))}</span><ul>${cg.unresolvedQuestions.map((t) => `<li>${esc(t)}</li>`).join('')}</ul></div>` : ''}
      ${cg.remainingConcerns.length ? `<div class="pdf-stance"><span class="pdf-stance-label">${esc(tr('cg.remainingConcerns', 'Remaining concerns'))}</span><ul>${cg.remainingConcerns.map((t) => `<li>${esc(t)}</li>`).join('')}</ul></div>` : ''}
