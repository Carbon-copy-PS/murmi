...
export function buildSummary(ctx) {
  ...
  cgHistory.forEach((item, index) => {
    const cg = commonGroundBlock(item)
    if (!cg) return
    out.push('', `AI COMMON GROUND v${index + 1}`, '----------------', cg.statement)
    if (cg.generatedAt) out.push(`Generated: ${new Date(cg.generatedAt * 1000).toLocaleString()}`)
    if (cg.generatedByName) out.push(`By: ${cg.generatedByName}`)
...
export function buildPrintableHTML(ctx) {
  ...
  cgHistory.forEach((item, index) => {
    ...
    const metaLine = [
      cg.generatedAt ? new Date(cg.generatedAt * 1000).toLocaleString() : null,
      cg.generatedByName ? `by ${esc(cg.generatedByName)}` : null,
      cg.depth,
    ].filter(Boolean).join(' · ')
    sections.push(`<section class="pdf-section pdf-cg">
      <h2><span class="pdf-kicker">AI Common Ground v${index + 1}</span></h2>
      ...
      ${cg.bridgingAlternatives.length ? `<div class="pdf-stance"><span class="pdf-stance-label">Alternative bridges</span><ul>${cg.bridgingAlternatives.map((t) => `<li>${esc(t)}</li>`).join('')}</ul></div>` : ''}
      ${cg.insights.length ? `<div class="pdf-stance"><span class="pdf-stance-label">Insights</span><ul>${cg.insights.map((t) => `<li>${esc(t)}</li>`).join('')}</ul></div>` : ''}
      ${cg.tradeoffs.length ? `<div class="pdf-stance"><span class="pdf-stance-label">Trade-offs</span><ul>${cg.tradeoffs.map((t) => `<li>${esc(t)}</li>`).join('')}</ul></div>` : ''}
      ${cg.groupNotes.length ? `<div class="pdf-stance"><span class="pdf-stance-label">Group perspectives</span><ul>${cg.groupNotes.map((g) => `<li><strong>Group ${esc(g.group)}:</strong> ${esc(g.note)}</li>`).join('')}</ul></div>` : ''}
